...
def _eta_to_sample_weights(eta: pd.Series, floor: float = 0.2, cap: float = 3.0) -> np.ndarray:
    """
    Map η values to strictly positive sample weights.

    We normalise η, map it linearly around 1.0, and clip to [floor, cap].
    Higher η (high goodput, low RTT/loss) receives higher weight so that
    updates associated with good operating points influence the model more.
    """
    eta_np = eta.to_numpy(dtype=float)
    if eta_np.size == 0:
        return np.array([], dtype=float)

    mean = float(np.mean(eta_np))
    std = float(np.std(eta_np)) or 1.0
    z = (eta_np - mean) / std
...
def train_model_with_eta_selection(
    ...
    # Ensure η is available for weighting and scoring.
    if "eta" not in df.columns:
        df = df.copy()
        df["eta"] = compute_eta(df, alpha=alpha, beta=beta)

    # Create a time-ordered view of the training data for internal val split.
    train_df = df[df["split"] == "train"].dropna(subset=feature_cols + ["y", "eta"]).copy()
    ...
    if val_df.empty:
        ...
        val_df = df[df["split"] == "test"].dropna(subset=feature_cols + ["y", "eta"]).copy()

    inner_eta = inner_train_df["eta"]
    eta_val = val_df["eta"]
    ...
    for params in param_grid:
        ...
        # Use η-derived sample weights so that the effective training loss
        # depends directly on the assignment's η(t-1) objective.
        train_weights = _eta_to_sample_weights(inner_eta)

        gb.fit(inner_train_df[feature_cols], inner_train_df["y"], sample_weight=train_weights)
    ...
    # Refit on the full training set with best params, again using η-based
    # sample weights so that η is part of the final training objective.
    full_train_df = df[df["split"] == "train"].dropna(subset=feature_cols + ["y", "eta"]).copy()
    full_weights = _eta_to_sample_weights(full_train_df["eta"])
    ...
    final_model.fit(full_train_df[feature_cols], full_train_df["y"], sample_weight=full_weights)
...
def main() -> None:
    ...
    # Compute η for all rows so it is available for training and evaluation.
    global_df["eta"] = compute_eta(global_df, alpha=args.alpha, beta=args.beta)

    # Train model using an η-influenced objective (η-weighted MSE) and
    # η-based validation score.
