...
# ---------------------------------------------------------------
# Step 0: Avoid the system network manager from messing up
# ---------------------------------------------------------------
echo ">>> Step 0: Preventing network manager interference..."
...
# ---------------------------------------------------------------
echo ""
echo ">>> Step 4: Disabling reverse path filtering..."
sysctl -w "net.ipv4.conf.${IFACE}.rp_filter=0" > /dev/null
sysctl -w net.ipv4.conf.all.rp_filter=0 > /dev/null
sysctl -w net.ipv4.conf.default.rp_filter=0 > /dev/null
echo "    rp_filter = 0 (all, default, ${IFACE})"

# Disable ICMP because it was causing issues with the ring routing
...
# ---------------------------------------------------------------
echo ""
echo ">>> Step 7: Setting static ARP entry..."

# For my classmates:
# - ip neigh replace "${RIGHT_IP}" lladdr "${RIGHT_MAC}" nud permanent dev "${IFACE}"
# - sets a permanent ARP entry for the right neighbor
# - nud permanent means that the ARP entry will not expire

ip neigh replace "${RIGHT_IP}" lladdr "${RIGHT_MAC}" nud permanent dev "${IFACE}"
echo "    ${RIGHT_IP} -> ${RIGHT_MAC} (permanent)"
...
echo "  IP:        ${MY_IP}"
echo "  MAC:       ${MY_MAC}"
echo "  Next hop:  ${RIGHT_IP} (MAC: ${RIGHT_MAC})"
echo ""
