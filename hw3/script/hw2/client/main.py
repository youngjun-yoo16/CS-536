...
def main():
    ...
    if results:
        ...
        # Compute per-result RTT and loss from intervals
        for r in results:
            intervals = r.get('intervals', [])
            if intervals:
                rtt_values = [iv['rtt_ms'] for iv in intervals if 'rtt_ms' in iv]
                lost_values = [iv.get('lost', 0) for iv in intervals]
                r['avg_rtt_ms'] = sum(rtt_values) / len(rtt_values) if rtt_values else 0.0
                r['total_lost'] = max(lost_values) if lost_values else 0  # cumulative counter
                r['total_retransmits'] = max(
                    (iv.get('retrans', 0) for iv in intervals), default=0
                )
            else:
                r['avg_rtt_ms'] = 0.0
                r['total_lost'] = 0
        ...
        total_retransmits = sum(r['total_retransmits'] for r in results)
        ...
        print(f"\nDetailed Results:")
        print(f"{'Host':<35} {'Country':<10} {'Tput (Mbps)':<14} {'RTT (ms)':<12} {'Lost':<8} {'Retrans':<8}")
        print("-" * 87)
        for r in sorted(results, key=lambda x: x['throughput_mbps'], reverse=True):
            print(f"{r['host']:<35} {r['country']:<10} {r['throughput_mbps']:>12.2f}  {r['avg_rtt_ms']:>10.2f}  {r['total_lost']:>6}  {r['total_retransmits']:>6}")
    else:
