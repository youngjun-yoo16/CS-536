...
- `-v ~/Downloads/CS536:/workspace` mounts your repo into `/workspace` so that
  any outputs (CSVs, PDFs) are written back to your host filesystem.
- You should see `hw1` and `hw2` inside `/workspace`.

### 3. Generate iperf traces (Q1/Q2/Q3 data)

Inside the container:

```bash
cd /workspace/hw2/client

# Run the iperf client against 5 random servers for 10 seconds each
python3 main.py -n 5 -d 10
```

This:

- Loads the server list from `listed_iperf3_servers.csv` (193 servers).
- Picks 5 random destinations.
- Automatically replaces servers that misbehave (e.g., bad greeting bytes) or
  refuse connections.
- Writes one CSV per successful destination:

```text
37.19.218.65_5201.csv
156.146.53.53_5201.csv
a205.speedtest.wobcom.de_5201.csv
speedtest.mia11.us.leaseweb.net_5201.csv
speedtest.wobcom.de_5201.csv
```

Each CSV contains timestamped samples every ~200 ms with TCP_INFO fields and
goodput, plus interval duration.

### 4. Run the ML pipeline (Q3)

Still inside `/workspace/hw2/client`:
...
1. Discover all trace CSVs in `--traces-dir`.
2. Build the global dataset and perform per‑trace 70/30 time‑based splits.
3. Train a Gradient Boosting model on Δcwnd, selecting hyperparameters using
   the η‑based objective.
4. Print test MSE and feature importances.
5. Generate cwnd vs time plots for 5 representative traces in
   `./q3_outputs/`.

Because of the bind mount, the outputs will also appear on your host at:

```text
hw2/client/q3_outputs/*.pdf
hw2/client/*.csv
```
...
The container is removed (`--rm`), but all generated data remains in your
